...
public interface JSONataInterface {
    ...
    @Schema(
        ...
        description = """
            ...
            1,000,000 records needs no more than a batch of 10. Raise it only for expressions with proven \
            deep recursion needs — each level adds frames to the chain traversed by variable lookup, so very \
            high values trade the clean error for a JVM StackOverflowError."""
    )
    @NotNull
    @PluginProperty(group = "main")
